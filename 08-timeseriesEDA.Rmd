...
Because the fake data (`df_ts`) used in previous chapters has no time series patterns, we'll use it alongside a data set with clear patterns (`beer`) so we can explore what these EDA tools show with and without time-related patterns.  
...
Using the `beer` data shows the partial autocorreltaion pattern. The spike at the second line indicates that there is a moderate negative relationship in values 6 months apart, and the spike at the fourth line shows there's a strong positive relationship in values 1 year (4 months) apart.  

...
When you have a trend like that seen above, you'll generally wait until the process has settled to a new, stable mean, and reset the central line accordingly.

But sometimes you have a trend that lasts for a long time, or is continuously trending. You can difference the data to remove the trend (create a new dataset by subtracting the value at time *t* from the value at time *t+1*), or you can use regression residuals to show deviations from the trend, but either approach can make it harder to interpret. Perhaps a better idea is use quantile regression to obtain the median line, which allows you to keep the data on the original scale.  
...
**Process shift "test"**: $log_2(n) + 3$ where $n$ is 33 points that do not touch the median is `r log2(33) + 3`, so there should be no more than 8 points in any given run. The most points in a run in this run chart is five.    
